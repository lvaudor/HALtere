...
function(input, output, session) {
  r_collection <- reactive({
    if(!("name_collection" %in% names(input))){
      collection="BIOEENVIS"
    }else{
      if(!input$new_collection){
        collection=input$name_collection
      }else{
        collection=input$name_new_collection
      }
    }
    collection
  })
  output$ui_collection_selection=renderUI({
    result=selectInput("name_collection",
                       "Collection:",
                       list.files("data"))
    if(input$new_collection){
      result=tagList(textInput("name_new_collection",
                               "Collection:",
                               value=""),
  ...
  r_get_ref_authors=reactive({
    datadir=paste0("data/",r_collection())
  ...
  observeEvent(r_collection(),{
    datadir=r_collection()
