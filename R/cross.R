...
#' @export
#' @examples
#' HALtere::cross("BIOEENVIS","data", type="people")
cross=function(collection, data_dir, type="people"){
  # Retrieve data_ref_authors
  data_ref_authors=readRDS(glue::glue("{data_dir}/{collection}/data_ref_authors.RDS"))
  # Retrieve data_groups
  data_groups=readRDS(glue::glue("{data_dir}/{collection}/data_{type}.RDS"))
  ...
  crossed_data=data_ref_authors%>%
    ...
    saveRDS(crossed_data,glue::glue("{data_dir}/{collection}/crossed_{type}.RDS"))
  return(crossed_data)
}
